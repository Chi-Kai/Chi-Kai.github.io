...
给定一个由可疑模型生成的个性化图像 $x_{s}$x\_s ，我们可以通过将其投影到 $\Phi$ 对应的特征空间来确定它是否包含水印。具体的，我们会计算该图像到超球中心的距离 $s \left(\right. x_{s} \left.\right) = \parallel \Phi \left(\right. x_{s} \left.\right) - o \parallel_{2}^{2}$s(x\_s)=\\|\\Phi(x\_s)-o\\|\_2^2 ，我们称之为涂层得分（Coating Score）。**理想情况下，侵权模型生成的个性化图片对应的涂层分数较小，而良性模型对应的涂层分数较大**。
